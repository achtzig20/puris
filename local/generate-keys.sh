...
SUPPLIER_PURIS_DTR_EDC_ASSET_REGISTER=true
...
# Bruno process .env generation in collection root
BRUNO_COLLECTION_ROOT="$(dirname "$(dirname "$BRUNO_FILE")")" 
BRUNO_DOTENV="${BRUNO_COLLECTION_ROOT}/.env"
mkdir -p "$BRUNO_COLLECTION_ROOT"

# Backup existing .env (timestamped)
if [ -f "$BRUNO_DOTENV" ]; then
  cp "$BRUNO_DOTENV" "${BRUNO_DOTENV}.bak.$(date +%Y%m%d%H%M%S)"
fi

# Write fresh secrets (only sensitive values)
tmp_env="$(mktemp)"
{
  echo "# Auto-generated by generate-keys.sh"
  echo "# Consumed in Bruno. Do NOT commit this file."
  echo
  echo "CUSTOMER_EDC_API_KEY=$EDC_API_PW"
  echo "CUSTOMER_PURIS_BACKEND_API_KEY=$CUSTOMER_BACKEND_API_KEY"
  echo "CUSTOMER_MANAGE_CLIENT_SECRET=$CUSTOMER_KC_DTR_PURIS_CLIENT_SECRET"
  echo "SUPPLIER_EDC_API_KEY=$EDC_API_PW"
  echo "SUPPLIER_PURIS_BACKEND_API_KEY=$SUPPLIER_BACKEND_API_KEY"
  echo "SUPPLIER_MANAGE_CLIENT_SECRET=$SUPPLIER_KC_DTR_PURIS_CLIENT_SECRET"
} > "$tmp_env"
mv "$tmp_env" "$BRUNO_DOTENV"
chmod 600 "$BRUNO_DOTENV"
echo "Bruno process .env written to: $BRUNO_DOTENV"

# Ensure .env is git-ignored inside the Bruno collection
if [ ! -f "${BRUNO_COLLECTION_ROOT}/.gitignore" ] || ! grep -qE '(^|/)\.env$' "${BRUNO_COLLECTION_ROOT}/.gitignore"; then
  echo ".env" >> "${BRUNO_COLLECTION_ROOT}/.gitignore"
fi

# Update the properties in the bruno file
if [ -f "$BRUNO_FILE" ]; then
  sed -i 's|^  CUSTOMER_EDC_API_KEY: .*|  CUSTOMER_EDC_API_KEY: {{process.env.CUSTOMER_EDC_API_KEY}}|' "$BRUNO_FILE"
  sed -i 's|^  CUSTOMER_PURIS_BACKEND_API_KEY: .*|  CUSTOMER_PURIS_BACKEND_API_KEY: {{process.env.CUSTOMER_PURIS_BACKEND_API_KEY}}|' "$BRUNO_FILE"
  sed -i 's|^  CUSTOMER_MANAGE_CLIENT_SECRET: .*|  CUSTOMER_MANAGE_CLIENT_SECRET: {{process.env.CUSTOMER_MANAGE_CLIENT_SECRET}}|' "$BRUNO_FILE"
  sed -i 's|^  SUPPLIER_EDC_API_KEY: .*|  SUPPLIER_EDC_API_KEY: {{process.env.SUPPLIER_EDC_API_KEY}}|' "$BRUNO_FILE"
  sed -i 's|^  SUPPLIER_PURIS_BACKEND_API_KEY: .*|  SUPPLIER_PURIS_BACKEND_API_KEY: {{process.env.SUPPLIER_PURIS_BACKEND_API_KEY}}|' "$BRUNO_FILE"
  sed -i 's|^  SUPPLIER_MANAGE_CLIENT_SECRET: .*|  SUPPLIER_MANAGE_CLIENT_SECRET: {{process.env.SUPPLIER_MANAGE_CLIENT_SECRET}}|' "$BRUNO_FILE"
  echo "Updated $BRUNO_FILE."
else
  echo "WARNING: $BRUNO_FILE not found; skipping local.bru update."
fi
