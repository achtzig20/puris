...
from dsp.common import CONTEXT_DSPACE, CONTEXT_DSPACE_ODRL_POLICY
...
async def negotiate_and_send_notification(
    ...
    asset_id, offer = await _fetch_notification_asset(
...
async def handle_agreement_and_send_verification(
    consumer_pid: str,
    body: dict,
    supplier_dsp_url: str,
    bpnl: str,
    ...
    """Handle ContractAgreementMessage and send verification back to supplier DSP."""
    ...
    provider_pid = body.get("dspace:providerPid")
    ...
    # Send verification asynchronously so we return 200 quickly.
    asyncio.create_task(
        _send_verification(supplier_dsp_url, provider_pid, consumer_pid, bpnl, supplier_bpnl, wallet_url, wallet_secret)
    )
    return True
...
async def _fetch_notification_asset(
    ...
    token = await _get_iatp_token(bpnl, supplier_bpnl, wallet_url, wallet_secret)
...
async def _get_fresh_access_token(
    ...
    si_token = await _get_iatp_token(bpnl, supplier_bpnl, wallet_url, wallet_secret)
    if not si_token:
