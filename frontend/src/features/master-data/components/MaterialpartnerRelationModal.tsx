            ...
            <Stack padding="0 2rem 2rem" sx={{ width: '60rem' }}>
                <Grid container spacing={1} padding=".25rem">
                    <>
                        ...
                        {showStockingSites && (
                            <Grid item xs={12}>
                                <InputLabel>Stocking Sites</InputLabel>
                                <Autocomplete
                                    id="own-stocking-sites"
                                    multiple
                                    options={siteOptions}
                                    value={temporaryMpr.ownStockingSiteBpnss ?? []}
                                    getOptionLabel={getSiteLabel}
                                    isOptionEqualToValue={(o, v) => o === v}
                                    renderInput={(params) => (
                                        <Input {...params} hiddenLabel placeholder="Select Stocking Sites of Partner" />
                                    )}
                                    onChange={(_, value) =>
                                        setTemporaryMpr((prev) => ({
                                            ...prev,
                                            ownStockingSiteBpnss: value ?? [],
                                        }))
                                    }
                                    />
                            </Grid>
